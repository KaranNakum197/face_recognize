...
class Face_Recognizer:
    def __init__(self):
        ...
        # Dictionary to track the first detection time of each student
        self.student_presence_time = {}
                    ...
                    if self.current_frame_face_cnt == 0:
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                # Track the first detection time of the student
                                nam = self.face_name_known_list[similar_person_num]
                                if nam not in self.student_presence_time:
                                    self.student_presence_time[nam] = time.time()
                                    print(f"First detection of {nam} at {time.ctime(self.student_presence_time[nam])}")
                                else:
                                    # Check if the student has been present for at least 10 minutes
                                    if time.time() - self.student_presence_time[nam] >= 6:  # 600 seconds = 10 minutes
                                        self.attendance(nam)
                                        del self.student_presence_time[nam]  # Remove the student from tracking
                                        print(f"{nam} has been present for 10 minutes. Attendance marked.")
                            else:
